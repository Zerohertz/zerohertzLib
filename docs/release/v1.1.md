...
- PyTest 실행 시 `api` 함수들에서 `HTTP 429 Too Many Requests` 오류를 방지하기 위해 `TIME_SLEEP` 범위 확대## v1.1.8
...
- `vision`
    ...
    - `YoloLoader` class에서 label studio format으로 변환하는 method의 `value` 수정## v1.1.7
...
- `api`
    - `GitHub` class의 release note 생성 시 merged가 아닌 closed 상태의 pull request를 제외하도록 수정 (→ <s></s>오류로 누락<s></s> → `git rebase`로 복구 완료)## v1.1.5
...
+ `pyproject.toml`의 format 변경## v1.1.4
...
+ CI/CD pipeline을 Jenkins에서 GitHub Actions로 migration## v1.1.3
...
+ `api`
    + `GitHub` class의 `release_note` method 사용 시 취소선이 제대로 표현되지 않는 현상 수정## v1.1.0
